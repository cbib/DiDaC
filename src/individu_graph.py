...
class individu_graph:
			...
			for record_s in SeqIO.parse(f, "fastq", generic_dna):
				...
				for i2 in range(0,len(sequence)-k):
					...
					if next_kmer not in dbg:
				 		dbg.add_node(next_kmer,read_list_n=[fragment+"_"+str(comp)],fragment=[fragment])
					if curr_kmer in dbg:
						dbg.node[curr_kmer]['read_list_n'].append(fragment+"_"+str(comp))
						...
						dbg.add_node(curr_kmer,read_list_n=[fragment+"_"+str(comp)],fragment=[fragment])
						dbg.add_edge(curr_kmer,next_kmer)
	...
	def graph_cleaned_init(self,X):
		...
		for n in self.dbg:
			...
			if len(set(self.dbg.node[n]['read_list_n'])) <= total_coverage*X/100:
