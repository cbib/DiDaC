...
def merge_reference_graph(reference_graph):
	...
	for bunch in buckets:
		entry_point=reference_graph.predecessors_iter(itertools.ifilter(is_entry_point,bunch).next()).next() # we know there's exactly 1 predecessor and thus 1 entry_point whose pred is in the graph 
		exit_point=reference_graph.successors_iter(itertools.ifilter(is_exit_point,bunch).next()).next() # we know there's exactly 1 successor and thus 1 exit point  whose succ exists in the graph
		...
		n_buckets+=1
	print "Found and compacted %d linear chains"%(n_buckets)
	print "Reducing graph size from %d to %d"%(len(reference_graph),len(merged_graph))
	return (merged_graph)
...
def merge_individu_graph(input_graph,reference_graph):
	...
	# Compact them in the merged graph
	for bunch in buckets:
		entry_point=input_graph.predecessors_iter(itertools.ifilter(is_entry_point,bunch).next()).next() # we know there's exactly 1 predecessor and thus 1 entry_point whose pred is in the graph 
		exit_point=input_graph.successors_iter(itertools.ifilter(is_exit_point,bunch).next()).next() # we know there's exactly 1 successor and thus 1 exit point  whose succ exists in the graph
		# print "Compacting chain (unordered): %s"%" / ".join(bunch)
		meta_node_lbl="_".join(bunch)
		...
		n_buckets+=1
	print "Found and compacted %d linear chains"%(n_buckets)
	print "Reducing graph size from %d to %d"%(len(input_graph),len(merged_graph))
	return (merged_graph)
