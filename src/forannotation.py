...
def alteration_list_to_transcrit_mutation(g_test,g_ref):
	for i_alteration in range(0, len(g_test.significant_alteration_list)):
		...
		if len(compact_cigard) == 6:
			...
			elif "NM_001126113.2" in g_ref.node[g_test.significant_alteration_list[i_alteration].reference_path[0]]['ref_list']:
			...
			if alteration_type == "X":
				...
				print "%s:c.%d%s>%s\t%d\t%d\t%f\t%f"%(splicing_variant_converter(splicing_variant),position,reference,alteration,g_test.significant_alteration_list[i_alteration].reference_read_count,g_test.significant_alteration_list[i_alteration].alternative_read_count,g_test.significant_alteration_list[i_alteration].pvalue_ratio,g_test.significant_alteration_list[i_alteration].zscore)
			elif alteration_type == "D":
				# c.76_78delACT
				reference = g_test.significant_alteration_list[i_alteration].reference_sequence[compact_cigard[0]:compact_cigard[0]+compact_cigard[2]]
				position = g_ref.node[g_test.significant_alteration_list[i_alteration].reference_path[0]]['ref_list'][splicing_variant]+compact_cigard[0]
				print "%s:c.%d_%ddel%s\t%d\t%d\t%f\t%f"%(splicing_variant_converter(splicing_variant),position,position+len(reference)-1,reference,g_test.significant_alteration_list[i_alteration].reference_read_count,g_test.significant_alteration_list[i_alteration].alternative_read_count,g_test.significant_alteration_list[i_alteration].pvalue_ratio,g_test.significant_alteration_list[i_alteration].zscore)
			else:
				# c.76_77insG
				alteration = g_test.significant_alteration_list[i_alteration].alternative_sequence[compact_cigard[0]:compact_cigard[0]+compact_cigard[2]]
				position = g_ref.node[g_test.significant_alteration_list[i_alteration].reference_path[0]]['ref_list'][splicing_variant]+compact_cigard[0]
				print "%s:c.%d_%dins%s\t%d\t%d\t%f\t%f"%(splicing_variant_converter(splicing_variant),position,position+1,alteration,g_test.significant_alteration_list[i_alteration].reference_read_count,g_test.significant_alteration_list[i_alteration].alternative_read_count,g_test.significant_alteration_list[i_alteration].pvalue_ratio,g_test.significant_alteration_list[i_alteration].zscore)
		else:
